...
public abstract class FileCommands {
    ...
    public boolean onOptionsItemSelected(int id) {
        if (id == R.id.add) {
        	Log.d("yuhao", "FileCommands----------EntryActivity-----------");
    ...
    private void showMessageDialog(int resId, DialogInterface.OnClickListener callback) {
	AlertDialog.Builder builder = new AlertDialog.Builder(mActivity);
	builder.setMessage(resId);
	...
	showMessageDialog(String.format(mActivity.getString(resId), file.toString()));
    }

    private void showPromptDialog(int title, int positiveButton, CharSequence text, final PromptCallback positiveCallback, DialogInterface.OnClickListener neutralCallback) {
	View view = mActivity.getLayoutInflater().inflate(R.layout.prompt, null);
	((TextView)view.findViewById(R.id.label)).setText(mActivity.getString(R.string.file_input));
	...
	Toast.makeText(mActivity, String.format(mActivity.getString(resId), file.toString()), Toast.LENGTH_SHORT).show();
    }
    ...
    private void sendFile(File file, String type) {
        ...
        mActivity.startActivity(Intent.createChooser(intent, mActivity.getString(R.string.share_file)));
    }
	...
	showPromptDialog(R.string.import_csv, R.string.import_, new File(Environment.getExternalStorageDirectory(), "weight.csv").toString(), new PromptCallback() {
	...
	try {
	    ...
	    Pattern pattern = Pattern.compile("^\\s*([^,;]+)\\s*[,;]\\s*(\\d+\\.?\\d*)\\s*([,;].*|)");
		...
		for (EntryData entry : list) {
		    mDatabase.exec("INSERT INTO weight (weight, created_at) VALUES (?, ?)", new Object[] { entry.weight, entry.created_at });
		}
	...
	final Cursor cursor = mDatabase.query("SELECT weight, created_at FROM weight ORDER BY created_at");
	...
	showPromptDialog(R.string.export_csv, R.string.export, new File(Environment.getExternalStorageDirectory(), "weight.csv").toString(), new PromptCallback() {
		public void run(String text) {
		    try {
    ...
    private void exportCsv(File file, Cursor cursor) {
        ...
        try {
            ...
            while (cursor.moveToNext()) {
                writer.write(format.format(new Date(1000L * cursor.getInt(1))) + "," + cursor.getInt(0) / 10.);
                writer.newLine();
            }
	...
	showPromptDialog(R.string.export_ods, R.string.export, new File(Environment.getExternalStorageDirectory(), "weight.ods").toString(), new PromptCallback() {
		public void run(String text) {
		    try {
	...
	try {
	    ZipOutputStream output = new ZipOutputStream(new BufferedOutputStream(new FileOutputStream(file)));
		...
		bufferedWriter.write("</text:p></table:table-cell>"
			+ "<table:table-cell office:value-type=\"float\" office:value=\"");
		String weight = "" + cursor.getInt(0) / 10.;
		bufferedWriter.write(weight);
		bufferedWriter.write("\"><text:p>");
		bufferedWriter.write(weight);
		bufferedWriter.write("</text:p></table:table-cell></table:table-row>");
	    }

	    bufferedWriter.write("</table:table></office:spreadsheet></office:body>"
		    + "</office:document-content>");
	...
	final View view = mActivity.getLayoutInflater().inflate(R.layout.export_png, null);
	...
	fileInput.setText(new File(Environment.getExternalStorageDirectory(), "weight.png").toString());
	...
	endDateButton.setText(DateFormat.getDateFormat(mActivity).format(date.getTime()));
	endDateButton.setOnClickListener(new View.OnClickListener() {
			...
			}, date.get(GregorianCalendar.YEAR), date.get(GregorianCalendar.MONTH), date.get(GregorianCalendar.DATE)).show();
		}
	    });
	...
	new Runnable() {
		...
		builder.setPositiveButton(R.string.export, new DialogInterface.OnClickListener() {
			public void onClick(DialogInterface dialog, int which) {
			    ...
			    int sizeY;
			    try {
				sizeX = Integer.parseInt(sizeXInput.getText().toString());
				sizeY = Integer.parseInt(sizeYInput.getText().toString());
			    } catch (NumberFormatException e) {
				showMessageDialog(R.string.invalid_size, callback);
				return;
			    }
			    if (sizeX < 1 || sizeY < 1) {
				showMessageDialog(R.string.invalid_size, callback);
				...
				showMessageDialog(R.string.invalid_days, callback);
				return;
			    }
			    if (days < 1) {
				showMessageDialog(R.string.invalid_days, callback);
			    ...
			    exportPng(new File(fileInput.getText().toString()), sizeX, sizeY, date, days, ((CheckBox)view.findViewById(R.id.transparency)).isChecked());
			}
		    });
    ...
    private void exportPng(File file, int sizeX, int sizeY, GregorianCalendar date, int days, boolean transparency) {
	...
	cursor.close();
	showMessageDialog(String.format(mActivity.getResources().getQuantityString(R.plurals.entry_count, count), count));
    }
