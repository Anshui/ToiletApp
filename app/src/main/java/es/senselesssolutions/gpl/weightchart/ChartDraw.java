...
//    	Cursor cursor = mDatabase.query("SELECT weight, created_at FROM weight WHERE created_at >= " + startDate.getTimeInMillis() / 1000 + " AND created_at < " + endDate.getTimeInMillis() / 1000  + " ORDER BY created_at");
    	Cursor cursor = mDatabase.query("SELECT weight, created_at, user_id_name FROM weight WHERE created_at >= " + startDate.getTimeInMillis() / 1000 + " AND created_at < " + endDate.getTimeInMillis() / 1000 + " ORDER BY created_at");
    	int count = cursor.getCount();
    	Log.d("yuhao", "count=---------------"+count);
    	Log.d("yuhao", "cursor=------------"+cursor);
    	if (count > 0) {
    		// Prepare data	
