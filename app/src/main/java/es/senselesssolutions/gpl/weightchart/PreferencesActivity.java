...
public class PreferencesActivity extends PreferenceActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
    	...
    	findPreference("weight_goal").setSummary(preferences.getString("weight_goal", "")
    			+" "+findPreference("weight_unit").getSummary()  
    			);
    	findPreference("weight_max").setSummary(preferences.getString("weight_max", "")
    			+" "+findPreference("weight_unit").getSummary()  
    			);
    	updateHeight();
    	...
    	findPreference("weight_goal").setOnPreferenceClickListener(new Preference.OnPreferenceClickListener() {
    		public boolean onPreferenceClick(Preference preference) {
    			showDialog(DIALOG_GOAL);
    			return true;
    		}
    	});
    	findPreference("weight_max").setOnPreferenceClickListener(new Preference.OnPreferenceClickListener() {
    		public boolean onPreferenceClick(Preference preference) {
        ...
        findPreference("market_author").setOnPreferenceClickListener(new Preference.OnPreferenceClickListener() {
            public boolean onPreferenceClick(Preference preference) {
                startActivity(new Intent(Intent.ACTION_VIEW, Uri.parse(getString(R.string.market_url_author))));
                return true;
            }
        });
        ...
        return new AlertDialog.Builder(PreferencesActivity.this)
            ...
            .setPositiveButton(R.string.alert_dialog_ok, new DialogInterface.OnClickListener() {
                public void onClick(DialogInterface dialog, int whichButton) {
                	String summary = ((EditText)textEntryView.findViewById(R.id.ET_weight))
                		.getText().toString();
                	findPreference("weight_goal").setSummary(summary
                			+" "+findPreference("weight_unit").getSummary()                			
                			);
    ...
    private Dialog createMaxWeightDialog(){
        LayoutInflater factory = LayoutInflater.from(this);
        final View textEntryView = factory.inflate(R.layout.ad_max_weight, null);
        ((EditText)textEntryView.findViewById(R.id.ET_max_weight))
        	.setText(
        		PreferenceManager.getDefaultSharedPreferences(PreferencesActivity.this)
               	.getString("weight_max", "")
        	);
        return new AlertDialog.Builder(PreferencesActivity.this)
            //.setIcon(R.drawable.alert_dialog_icon)
            ...
            .setPositiveButton(R.string.alert_dialog_ok, new DialogInterface.OnClickListener() {
                public void onClick(DialogInterface dialog, int whichButton) {
                	String summary = ((EditText)textEntryView.findViewById(R.id.ET_max_weight))
                		.getText().toString();
                	findPreference("weight_max").setSummary(summary
                			+" "+findPreference("weight_unit").getSummary()                			
                			);
                	SharedPreferences.Editor edit = PreferenceManager.getDefaultSharedPreferences(PreferencesActivity.this).edit();
    ...
    private void updateWeightUnit(Object unit) {
    	findPreference("weight_unit").setSummary(
    			getResources().getStringArray(R.array.weight_unit_labels)["lb".equals(unit) ? 1 : "st".equals(unit) ? 2 : 0]);
    	SharedPreferences preferences = PreferenceManager.getDefaultSharedPreferences(this);
    	findPreference("weight_goal").setSummary(preferences.getString("weight_goal", null)
    			+" "+findPreference("weight_unit").getSummary()  
    			);
    	findPreference("weight_max").setSummary(preferences.getString("weight_max", null)
    			+" "+findPreference("weight_unit").getSummary()  
    			);
    }
