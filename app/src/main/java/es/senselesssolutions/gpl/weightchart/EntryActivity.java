...
public class EntryActivity extends Activity {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        Log.d("yuhao", "onCreate------------EntryActivity------------");
        ...
        if (savedInstanceState != null) {
            ...
            mId = getIntent().getLongExtra("es.senselesssolutions.gpl.weightchart.Id", 0);

            if (mId != 0) {
                Database database = new Database(EntryActivity.this);
                Cursor cursor = database.query("SELECT weight, created_at, user_id_name FROM weight WHERE _id = " + mId);
    ...
    protected Dialog onCreateDialog(int id) {
        ...
        if (id == 1) {
            ...
            }, mDateTime.get(GregorianCalendar.HOUR_OF_DAY), mDateTime.get(GregorianCalendar.MINUTE), DateFormat.is24HourFormat(this));
        }
        return null;
    }
    ...
    private void saveAndFinish() {
        ...
        String userName = Define.USER_ID_NAME;
        Log.d("yuhao", "Define.USER_ID_NAME=-------------"+Define.USER_ID_NAME);
        Log.d("yuhao", "createdAt=----------------"+createdAt);
        Database database = new Database(EntryActivity.this);

        if (mId == 0) {
            database.exec("INSERT INTO weight (weight, created_at, user_id_name) VALUES (?, ?, ?)", new Object[] { weight, createdAt, userName });
        } else {
            database.exec("UPDATE weight SET weight = ?, created_at = ?  user_id_name = ? WHERE _id = ?", new Object[] { weight, createdAt, userName, mId });
        }
